-- ============================================
-- COMPLETE DATABASE SCHEMA v4.8
-- ============================================
-- Date: 2026-01-24
-- Purpose: Complete current schema snapshot
-- Status: REFERENCE ONLY - Represents actual database state
-- 
-- This is a snapshot of the current production database.
-- Use this as baseline for v5.0 migration planning.
-- 
...
CREATE TABLE IF NOT EXISTS kara_users (
  ...
  auth_user_id UUID,  -- For Supabase Auth integration (currently unused)
...
CREATE TABLE IF NOT EXISTS kara_rooms (
  ...
  -- Payment & subscription fields (unused in v4.8, reserved for v5.0)
...
-- 6.4: kara_songs_set_base_fields (helper function for song metadata)
...
-- Expected output (v4.8):
-- kara_files             | 7 columns
-- kara_languages         | 3 columns  (missing created_at vs v3.1)
-- kara_queue             | 17 columns (added youtube_url, source_type, metadata, sort_key)
-- kara_room_participants | 10 columns (added role, status, approved_at, expires_at, user_name)
-- kara_rooms             | 15 columns (added approval_mode, primary_tv_id, current_song_started_at)
-- kara_song_groups       | 4 columns
-- kara_song_history      | 8 columns
-- kara_user_preferences  | 8 columns (has id + user_id unique, favorite_song_ids is JSONB)
...
WHERE routine_schema = 'public'
  AND routine_name IN ('advance_playback', 'host_reorder_queue', 'user_reorder_queue', 'kara_songs_set_base_fields')
