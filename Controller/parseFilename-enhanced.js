...
export function parseFilename(filename, storagePath = '') {
  ...
  for (const t of tokens) {
    // Skip tone detection if already set from explicit pattern (explicit takes priority)
    if (!tone) {
      // For duets, don't set tone if both "nam" and "nu" are present
      if (['nam', 'male'].includes(t)) {
        // Only set tone if not a duet with both genders
        if (!(isSongCa && joined.includes('nu'))) {
          tone = 'nam';
        }
        continue;
      }
      if (['nu', 'female', 'nữ'].includes(t)) {
        // Only set tone if not a duet with both genders
        // Be careful - "nu" might be part of Vietnamese words like "Nụ"
        // Only set if it's clearly a tone indicator (check context)
        if (!(isSongCa && joined.includes('nam'))) {
          // Check if this "nu" appears near "tone" (within 3 words)
          const nuIndex = joined.indexOf(` ${t} `);
          const toneIndex = joined.indexOf(' tone ');
          if (nuIndex > 0 && toneIndex > 0 && Math.abs(nuIndex - toneIndex) < 20) {
            tone = 'nu';
          }
        }
        continue;
